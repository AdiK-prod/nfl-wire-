-- Idempotent seed data for teams and sources

-- Seed teams (32 NFL teams). If a team already exists by slug, skip it.
...
-- Seed global sources (by URL), skipping if they already exist.
...
-- Seed Seahawks-specific sources by joining on slug; skip if URL already present.
insert into public.sources (team_id, url, name, type, status, relevance_score)
select
  t.id,
  s.url,
